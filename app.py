...
def display_book_page(db, book_id):
    ...
    all_copies = db.execute("""
        SELECT HardCopy.id AS copyId, dateBorrowed, dateReturned, dateDue, borrowerId
        FROM HardCopy LEFT JOIN Loan ON HardCopy.id = hardCopyId
        WHERE HardCopy.bookId = ? GROUP BY HardCopy.id ORDER BY copyId ASC
